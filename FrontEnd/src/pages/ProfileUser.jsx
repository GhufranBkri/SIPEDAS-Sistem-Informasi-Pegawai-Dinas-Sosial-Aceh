...
const ProfileUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3000/employees/${nip}`, {
          headers: {
  ...
  const renderValue = (key) => {
    ...
    if (key === "foto") {
      return value ? <img src={value} alt="Foto" className="w-32 h-32 items-center object-cover" /> : null;
    }
    return (
      <input
        type="text"
        ...
        <div className="form-1 bg-white shadow overflow-hidden sm:rounded-lg p-6">
          ...
          <div className="form-2 bg-white shadow-xl overflow-hidden sm:rounded-lg p-6 my-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                "nama", "foto", "jenis_kelamin", "tempat_lahir", "tanggal_lahir", "umur",
                "golongan_darah", "nik", "no_kk", "no_telepon", "no_rekening",
                "email_gov", "email"
              ].map((key) => (
          ...
          <div className="form-3 bg-white shadow-xl overflow-hidden sm:rounded-lg p-6 my-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                "kabupaten", "kecamatan", "desa", "jalan", "alamat_lengkap"
              ].map((key) => (
                <div className="mb-4" key={key}>
                  <label className="block text-gray-700 mb-2">
                    {key.replace("_", " ").toUpperCase()}
          ...
          <div className="form-5 bg-white shadow-xl overflow-hidden sm:rounded-lg p-6 my-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                "nip", "npwp", "bidang", "eselon", "sub_bidang", "jabatan_terakhir",
                "gol_ruang", "jenjang", "jenis", "jenis_tekon", "tahun_sk_awal",
                "tahun_sk_akhir", "masa_kerja", "Kelas_jabatan", "no_req_bkn"
              ].map((key) => (
                <div className="mb-4" key={key}>
                  <label className="block text-gray-700 mb-2">
                    {key.replace("_", " ").toUpperCase()}
